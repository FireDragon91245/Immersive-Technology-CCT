...
public class TileEntityFluidPumpAlternative extends blusunrize.immersiveengineering.common.blocks.metal.TileEntityFluidPump implements ITickable, IBlockBounds, IHasDummyBlocks, IConfigurableSides, IFluidPipe, IIEInternalFluxHandler, IBlockOverlayText, IEBlockInterfaces.IPlayerInteraction {
    ...
    public void readCustomNBT(NBTTagCompound nbt, boolean descPacket) {
        ...
        if (nbt.hasKey("computerOn", 1) && ITCompatModule.isAnyComputerModLoaded) {
    ...
    public int getRSPower(BlockPos position) {
        ...
        for (EnumFacing directions : EnumFacing.values())
            toReturn = Math.max(world.getRedstonePower(position.offset(directions, -1), directions), toReturn);
        return toReturn;
    }
    ...
    public void update() {
        ...
        if (tank.getFluidAmount() > 0 && computerOn.orElse(true)) {
            int i = outputFluid(tank.getFluid(), false);
            tank.drain(i, true);
        }

        if (computerOn.orElseGet(() -> getRSPower(getPos()) > 0 || getRSPower(getPos().add(0, 1, 0)) > 0)) {
            for (EnumFacing f : EnumFacing.values()) {
